...
uint8 KPD_u8GetPressedKey(void)
{
	...
	uint8 Local_au8KPDARR[ROW_NUM][COL_NUM]=KPD_BUTTON_ARR;
	/*initial (COLUMN->HIGH, ROW->INPUTPULLUP) state will handle in PORT_DRIVER */
	...
	for(Local_u8ColIdx=0u; Local_u8ColIdx<COL_NUM; Local_u8ColIdx++)
	{
		...
		for(Local_u8RowIdx=0u; Local_u8RowIdx<ROW_NUM; Local_u8RowIdx++)
		{
			...
			if(Local_u8PinState == DIO_u8PIN_LOW)
			{
				...
				/*Poling "Keep reading" with blocking "bec. use delay"(waiting) until the key is released*/
