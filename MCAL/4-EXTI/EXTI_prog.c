...
static void(*EXTI_pvCallBack[3])(void)={0};     /*pointer to function*/
...
uint8 EXTI_u8EnableInChannel(IntChannel_t Copy_IntCh)
{
	uint8 Local_u8ErrorState= OK;
...
uint8 EXTI_u8DisableInChannel(IntChannel_t Copy_IntCh)
{
	uint8 Local_u8ErrorState= OK;
...
uint8 EXTI_u8SetCallBack(IntChannel_t Copy_IntCh,void(*Copy_pvCallBackFunc)(void))
{
	...
	{
		Local_u8ErrorStates = NULL_PTR_ERR;
	}
